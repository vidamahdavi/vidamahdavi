Vida Mahdavi – Cybersecurity Portfolio  
🔐 Cybersecurity Analyst | Incident Response | Security Automation | NIST CSF

## 📝 Professional Statement
Cybersecurity professional completing a Master’s degree at Georgia Tech (August 2026) with hands-on experience in monitoring security events, analyzing logs, detecting threats, and responding to incidents. Skilled in security automation, with practical experience applying frameworks such as the NIST Cybersecurity Framework (CSF). Proficient in using Python, Linux, and SIEM tools to protect systems and data.

## 🛠 Skills

**Security & Analysis**
- Incident Response (NIST CSF lifecycle)
- Threat Detection & Analysis
- Network Traffic Analysis
- Intrusion Detection (IDS concepts)

**Tools & Technologies**
- SIEM: Splunk
- Network Analysis: Wireshark
- Operating Systems: Linux, Windows
- Programming: Python (automation), Bash

**Frameworks & Concepts**
- NIST Cybersecurity Framework (CSF)
- Risk Assessment & Security Auditing
- TCP/IP Networking
  
## 📂 Featured Projects

🔐 Security Audit & Risk Assessment (NIST CSF)
- Conducted a security audit using NIST Cybersecurity Framework (CSF)
- Identified risks, vulnerabilities, and compliance gaps
- Provided actionable security recommendations
  🔗 https://github.com/vidamahdavi/botium-toys-it-audit

🚨 Incident Response Analysis (DoS Attack)
- Analyzed a real-world DoS attack using NIST CSF phases
- Documented detection, response, and recovery strategies
- Improved network security posture through structured analysis
  🔗 https://github.com/vidamahdavi/incident-report-analysis

⚙️ IP Allow List Automation (Python)
- Developed a Python algorithm to manage access control
- Automated removal of unauthorized IP addresses
- Improved security efficiency and reduced manual errors
  🔗 https://github.com/vidamahdavi/ip-allow-list-automation

## 🎓 Georgia Tech Coursework

- GeorgiaTech Cybersecurity Projects (Honor Code Compliant)  
🔗 https://github.com/vidamahdavi/georgia-tech-cybersecurity-coursework

## 📜 Certifications
- Google Cybersecurity Professional Certificate  
- Automate Cybersecurity Tasks with Python
  
## 🎯 Current Focus
- Building hands-on cybersecurity projects  
- Strengthening SIEM and threat detection skills  
- Preparing for cybersecurity analyst roles  
  
## 📫 Contact
- 📧 Email: **vidamahdavi@gmail.com**  
- 🔗 [LinkedIn]:  https://www.linkedin.com/in/vida-mahdavi-463813241/ 

